...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                for agent_id in range(0,16):
                    with self.sess.as_default(): 
                        action[str(agent_id)] = self.act(np.array(obs[str(agent_id)])[None], update_eps=update_eps, **kwargs)[0]
                ...
                if minute_time % 3 != 0:
                    ...
                    for agent_id in range(0,16):
                        if flag_pre != 'None': 
                            if flag_pre[str(agent_id)] != -1:
                                action[str(agent_id)] = flag_pre[str(agent_id)]
                                #action[str(agent_id)] = random.randint(0, 1)
                #'''Selecting part-uncertainty
                                              
                env_action = action
                reset = False
             
                importance_weight_list, neighbors_list, time_on_phase_list, phase_id_list, step_num, veh_position_data, veh_waiting_time, veh_complete_data, total_new_obs, total_rew, done, info = self.env.step(env_action)
                ...
                for aa in sorted(importance_weight_list.keys()):
                    ...
                    if first_lane_weight > 10 or second_lane_weight > 10: 
                        if aa == '0': 
                    ...
                    if first_lane_weight <= 10 and second_lane_weight <= 10:  
                        flag[aa] = -1
                    ...
                    for bb in sorted(veh_complete_data[aa].keys()):
                        for cc in sorted(veh_complete_data[aa][bb].keys()):
                            for dd in sorted(veh_complete_data[aa][bb][cc].keys()):
                                for ee in sorted(veh_complete_data[aa][bb][cc][dd].keys(), reverse=True):
...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            ...
            outfile = open('DQN/State-Similarity-Reward/State-Similarity-Reward-Scenario2-DQN-result-OUR-ALGORITHM-run1.csv','w')
