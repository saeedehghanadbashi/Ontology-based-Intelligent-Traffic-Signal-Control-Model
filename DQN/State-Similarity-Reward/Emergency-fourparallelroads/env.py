...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        if self.single_agent:
            ...
            return  importance_weight_list, self.neighbors_list, time_on_phase_list, phase_id_list, self.counter, veh_position_data, veh_waiting_time, veh_complete_data, observation, reward, done['__all__'], {}
