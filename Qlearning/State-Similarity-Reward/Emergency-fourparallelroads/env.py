...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        if  experiment_time % 2 == 0:
            while self.ambulance_count < 5:
                traci.vehicle.add(str(self.counter + random_id_list[0+self.ambulance_count]), "fourparallelroads", "Ambulance", None, "random", "base", "0", "current", "max","current","","","",0, 0)
                self.ambulance_count = self.ambulance_count + 1

        if  experiment_time % 2 == 0:
            while self.fueltruck_count < 5:
                traci.vehicle.add(str(self.counter + random_id_list[5+self.fueltruck_count]), "fourparallelroads", "fueltruck", None, "random", "base", "0", "current", "max","current","","","",0, 0)
                self.fueltruck_count = self.fueltruck_count + 1

        if  experiment_time % 2 == 0:
            while self.trailer_count < 5:
                traci.vehicle.add(str(self.counter + random_id_list[10+self.trailer_count]), "fourparallelroads", "trailer", None, "random", "base", "0", "current", "max","current","","","",0, 0)
        ...
        if self.counter % 5 == 0 and self.ambulance_count_0to1 < 5:
        ...
        if self.counter % 5 != 0: self.ambulance_count_0to1 = 0
        '''
        ...
        if self.counter % 5 == 0 and self.ambulance_count_4to5 < 5:
            traci.vehicle.add(str(self.counter + random_id_list[5+self.ambulance_count_4to5]), "route4to5", "Ambulance", None, "first", "base", "0", "current", "max","current","","","",0, 0)
            self.ambulance_count_4to5 = self.ambulance_count_4to5 + 1

        if self.counter % 5 == 0 and self.ambulance_count_9to10 < 5:
            traci.vehicle.add(str(self.counter + random_id_list[10+self.ambulance_count_9to10]), "route9to10", "Ambulance", None, "random", "base", "0", "current", "max","current","","","",0, 0)
            self.ambulance_count_9to10 = self.ambulance_count_9to10 + 1

        if self.counter % 5 == 0 and self.ambulance_count_14to15 < 5:
            traci.vehicle.add(str(self.counter + random_id_list[15+self.ambulance_count_14to15]), "route14to15", "Ambulance", None, "first", "base", "0", "current", "max","current","","","",0, 0)
...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        if self.single_agent:
            ...
            return  ambulance_lane_list, trailer_lane_list, fueltruck_lane_list, importance_weight_list, self.neighbors_list, time_on_phase_list, phase_id_list, self.counter, veh_position_data, veh_waiting_time, veh_complete_data, observation, reward, done, {}
...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def _get_important_objects_lane_list(self):
        ...
        Return the current observation for each traffic signal
        """
        ambulance_lane_list = Dict()
        trailer_lane_list = Dict()
        fueltruck_lane_list = Dict()
        for ts in self.ts_ids: # ts means all traffic signals in the network       
            ambulance_lane_retval, trailer_lane_retval, fueltruck_lane_retval = self.traffic_signals[ts]._get_important_objects_lane_list()
            ambulance_lane_list [ts] = ambulance_lane_retval
            trailer_lane_list [ts] = trailer_lane_retval
            fueltruck_lane_list [ts] = fueltruck_lane_retval
