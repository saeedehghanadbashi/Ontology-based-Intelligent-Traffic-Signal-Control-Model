    ...
    def test_keep(self):
        ...
        for lane in self.lanes:
            ...
            if k in self.env.last_distance_reward:
                ...
                if self.env.last_distance_reward[k] == 0 and (k in ambulance_lane_list or k in trailer_lane_list or k in fueltruck_lane_list): give_up = 1
        return keep, give_up
        ...
        for key in veh_list_part1:
            ...
            for veh in veh_list_part1[key]:
                ...
                if veh_type == "Ambulance":
                    importance_weight_val_per_road[key] = importance_weight_val_per_road[key] + 10
                    cnt[key] = 1
                if veh_type == "fueltruck":
                    importance_weight_val_per_road[key] = importance_weight_val_per_road[key] + 5
                    cnt[key] = 1
                if veh_type == "trailer":
                    importance_weight_val_per_road[key] = importance_weight_val_per_road[key] + 2
                    cnt[key] = 1
                if veh_type == "DEFAULT_VEHTYPE" and traci.vehicle.getAccumulatedWaitingTime(veh) != 0:
                    importance_weight_val_per_road[key] = importance_weight_val_per_road[key] + 0
                    cnt[key] = 1
        for key in veh_list_part2:
            if cnt[key] == 0: importance_weight_val_per_road[key] = 0  
            for veh in veh_list_part2[key]:
