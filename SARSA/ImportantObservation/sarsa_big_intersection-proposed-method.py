...
if __name__ == '__main__':
    ...
    out_csv = 'SARSA/ImportantObservation/Important-Observation-SARSA-output-OUR-ALGORITHM-run1.csv' #+ experiment_time*
    ...
    env = SumoEnvironment(net_file='nets/4x4-Saeedeh/4x4.net.xml',
                          ...
                          max_green=100,
                          max_depart_delay=0,
                          tripinfo_output='SARSA/ImportantObservation/Important-Observation-SARSA-tripinfo-OUR-ALGORITHM-run1.xml',
...
#new added ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                ...
                if step_num_pre != 'None' and minute_time % 2 == 0:
                    for agent_id in agents.keys():
                        ...
                        if flag_pre != 'None': action[agent_id] = flag_pre[agent_id]
                        else: action[agent_id] = random.randint(0, 1)
                #'''Selecting part-uncertainty
            ...
            outfile = open('SARSA/ImportantObservation/Important-Observation-SARSA-result-OUR-ALGORITHM-run1.csv','w')
